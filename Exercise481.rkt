...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname Exercise481) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
...
(define (row n img)
  (cond
    [(equal? n 0) (rectangle (image-width img) 1 "outline" "red")]
    [else (above img
                 (row (- n 1) img))]))
(define (col n img)
  (cond
    [(equal? n 0) (rectangle 1 (image-height img) "outline" "red")]
    [else (beside img
                  (col (- n 1) img))]))
...
(check-expect (n-queens-solution? (n-queens 8) 8)
              #true)
(define (n-queens-solution? qps n)
  (and (equal? (length qps) n)
       (andmap (lambda (x)
                 (equal? (- n 1)
                         (length (not-threatening-with x (remove x qps)))))
               qps)))

(render-queens 8 (n-queens 8) (circle 2 "solid" "blue"))