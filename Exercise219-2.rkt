...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname Exercise219-2) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define G-width 30)
(define G-height 30)
(define BG-width (* Step G-width))
(define BG-height (* Step G-height))
(define BG (empty-scene BG-width BG-height))
(define Speed 1)
(define Worm [rectangle Step Step "solid" "red"])
(define Food [rectangle Step Step "outline" "green"])
...
(define (run-into-wall? worm)
  (not (and
        (<= 0 (posn-x (first (worm-lop worm))) G-width)
        (<= 0 (posn-y (first (worm-lop worm))) G-height))))
...
(define (place-imagesX img lop bg)
  (cond
    ...
    [else (place-image img
                       (* Step (posn-x (first lop)))
                       (* Step (posn-y (first lop)))
...
(define (main s)
  (length (worm-lop
           (world-worm
            (big-bang s
              [on-draw world-draw]
              [on-tick world-tick FPS]
              [stop-when world-stop? last-world]
              [on-key world-key])))))

(main (make-world (make-worm (list (make-posn 4 3))
                             "right")
                  (create-food-not (make-posn 1 1))))
