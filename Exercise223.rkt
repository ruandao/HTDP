...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname Exercise223) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (block-generate-not x1 x)
  (if (equal? x1 x)
      ...
      (make-block x1 0)))
(define (tetris-end w)
  (place-image (text (number->string (length (tetris-landscape w)))
                     17 "red")
               (/ SCENE-width 2) (/ SCENE-height 2)
               BG))
(define (tetris-stop? w)
  (and #true
      (and (not (empty? (tetris-landscape w)))
           (equal? (block-y (first (tetris-landscape w))) 0))))
(define (tetris-main rate)
  (big-bang (make-tetris (block-generate -1) '())
    [on-tick tetris-tick rate]
    [on-draw tetris-render]
    [stop-when tetris-stop? tetris-end]
    [on-key tetris-key]))
